...
public class ComprobadorCarta : MonoBehaviour {
	// creo los game object de las cartas ocultas y visibles
	...
	// variable auxiliar para que no se pueda pulsar mas de dos cartas sin comprobacion
	...
	public void inicioValoresEmpieceJuego  (){
		...
		// inicializo el valor para acceso a pulsar cartas. 
		palabraItem = false;
		// cartas acertadas
		cartasAcertadas = 0;
	...
	void calculoPosicionCartaOculta (){
		...
		while (i<12) {
			...
			if (posicionEnPanel == 0 && posicionUsada [0] == false) {
				elementosOcultos[i].transform.Translate ((float)(-1.6),(float)( 3), 0);
				posicionUsada[0]=true;
				situadaEnPosicion[0]= i;
				i++;
			}
			if (posicionEnPanel == 1 && posicionUsada [1] == false) {
				elementosOcultos[i].transform.Translate (0, (float)(3), 0);
			...
			if (posicionEnPanel == 3 && posicionUsada [3] == false) {
				elementosOcultos[i].transform.Translate ((float)(-1.6), 1, 0);
				posicionUsada[3]=true;
				situadaEnPosicion[3]= i;
				i++;
			}
			if (posicionEnPanel == 4 && posicionUsada [4] == false) {
				elementosOcultos[i].transform.Translate (0, 1, 0);
			...
			if (posicionEnPanel == 6 && posicionUsada [6] == false) {
				elementosOcultos[i].transform.Translate ((float)(-1.6),(float)(-1), 0);
				posicionUsada[6]=true;
				situadaEnPosicion[6]= i;
				i++;
			}
			if (posicionEnPanel == 7 && posicionUsada [7] == false) {
				elementosOcultos[i].transform.Translate (0,(float)(-1), 0);
